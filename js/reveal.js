...
export default function( revealElement, options ) {
	...
	function getSlidePastCount( slide = currentSlide ) {
		...
		mainLoop: for( let i = 0; i < horizontalSlides.length; i++ ) {
			...
			for( let j = 0; j < verticalSlides.length; j++ ) {
				...
				if( verticalSlides[j].dataset.visibility !== 'uncountedX' ) {
					pastCount++;
				}
			...
			if( horizontalSlide.classList.contains( 'stack' ) === false && horizontalSlide.dataset.visibility !== 'uncountedX' ) {
				pastCount++;
			}
	 ...
	 * @return {number} Past slide count
	 */
	function getSlidePastCountFSN( slide = currentSlide ) {
	...
	function getSlides() {
		...
		return Util.queryAll( dom.wrapper, SLIDES_SELECTOR + ':not(.stack):not([data-visibility="uncountedX"])' );
	 ...
	 * Retrieves all slides in this presentation.
	 */
	function getSlidesFSN() {

		return Util.queryAll( dom.wrapper, SLIDES_SELECTOR + ':not(.stack):not([data-visibility="uncounted"]):not([data-visibility="uncountedX"])' );
	 ...
	 * @return {number}
	 */
	function getTotalSlidesFSN() {

		return getSlidesFSN().length;
